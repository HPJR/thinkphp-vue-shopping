<template>
    <div>
        ...
        <div class="main-product wrapper">
            ...
            <!--pro-list-list-->
            <div class="pro-list" style="position: relative;">
                <sortType :sortList="list"></sortType>
                <!--分页-->
                <page :count="count" :listRows="listRows" @getPage="changePage" v-if="listRows"></page>
...
<script>
    import {getDate,searchSome} from '../../service/index';
    ...
    export default {
        ...
        methods:{
            ...
            //新品列表及页码
            async getNewList(){
                let result = await searchSome(this.$route.query.val);
                if(result.list){
        ...
        watch:{
            '$route'(to, from) {
                this.loadingStates.newList = true;
                //监听路由是否变化
                if(to.query.val !== from.query.val){
                    this.getNewList();//重新加载数据
                }
            }
        },
    };
